...
@Injectable()
export class PdfServiceCastration {
  constructor(
  ...
  generateRow(appointment: Appointment, id: number, values: string[]) {
    const columns: string[] = [(id + 1).toString()];
    if (values.includes('Dueño'))
      columns.push(appointment.lastName, appointment.name);
    ...
    if (values.includes('Paciente'))
      columns.push(appointment.castration?.animalName || '-');

    if (values.includes('Edad'))
      columns.push(appointment.castration?.age || '-');
    if (values.includes('Peso(KG)'))
      columns.push(appointment.castration?.weight?.toString() || '-');
    ...
    if (values.includes('Características'))
      columns.push(appointment.castration?.features || '-');
    if (values.includes('Observaciones'))
      columns.push(appointment?.castration?.observations || '-');
  ...
  async newTable(
    ...
    const headers = [{ label: 'N°' }];
    if (values.includes('Dueño'))
      headers.push({ label: 'Apellido' }, { label: 'Nombre' });
    values.map((c, i) => {
